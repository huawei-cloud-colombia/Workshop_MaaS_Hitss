...
Evolución: El agente único se ha refactorizado en un sistema de dos agentes
especializados que colaboran secuencialmente, cada uno con su toolkit MCP:
  - Agent for Analyze (MCP - Analyze): resume, categoriza y lee directrices.
  - Agent for Action (MCP - Action): propone solución y envía notificación Gmail.
...
# --- Prompt del Agente Analista (Paso 1) ---
SYSTEM_PROMPT_ANALYZE = """Eres un Agente Analista experto en Clasificación de \
Soporte. Tu único objetivo es entender e interpretar el ticket entrante.

Tienes acceso exclusivo al toolkit MCP - Analyze, que contiene las herramientas:
- `get_ticket_handling_guidelines`: Recupera las directrices corporativas. \
Consultala SIEMPRE primero.
- `summarize_ticket`: Genera un resumen ejecutivo del problema.
- `categorize_ticket`: Determina la categoría y prioridad del ticket.

Flujo obligatorio:
1. Consulta `get_ticket_handling_guidelines` para obtener las políticas de la empresa.
2. Usa `summarize_ticket` para generar un resumen ejecutivo.
3. Usa `categorize_ticket` para determinar categoría y prioridad.

**IMPORTANTE:** Después de ejecutar las herramientas, devuelve ÚNICAMENTE \
...

{
  ...
  "categoria": "Técnico|Bug|Facturación|Solicitud de feature",
  "prioridad": "Alta|Media|Baja",
  "guidelines": "Directrices corporativas recuperadas (resumidas)."
}
"""

# --- Prompt del Agente Ejecutor (Paso 3) ---
SYSTEM_PROMPT_ACTION = """Eres un Agente Ejecutor de Soporte. Tu objetivo es \
tomar el análisis estructurado de un ticket y proceder con la resolución y \
comunicación.

Tienes acceso exclusivo al toolkit MCP - Action, que contiene las herramientas:
- `propose_solution`: Genera la propuesta de solución corporativa final, \
consumiendo el análisis previo del Agente Analista.
- `notify_via_gmail`: Envía la notificación real por correo electrónico.

Flujo obligatorio:
1. Usa `propose_solution` para generar la respuesta propuesta, utilizando como \
contexto las directrices y el análisis del Agente Analista.
2. Usa `notify_via_gmail` para enviar la notificación por correo al finalizar.
...
# ---------------------------------------------------------------------------
# Toolkit MCP - Analyze: summarize, categorize, get_guidelines
MCP_TOOLS_ANALYZE = [
    {
        "type": "function",
        "function": {
...
# Toolkit MCP - Action: propose_solution, notify_via_gmail
MCP_TOOLS_ACTION = [
    {
        "type": "function",
        "function": {
            ...
            "description": (
                ...
                "contexto las directrices de documentación corporativa y el "
                "análisis previo del Agente Analista."
            ),
            "parameters": {
                "type": "object",
                "properties": {
...
# Mapa de nombres de tools a funciones ejecutables — Toolkit Analyze
TOOL_FUNCTIONS_ANALYZE = {
...
# Mapa de nombres de tools a funciones ejecutables — Toolkit Action
TOOL_FUNCTIONS_ACTION = {
...
def _run_agent(
    ...
    """
    Ejecuta un bucle de Tool Calling (MCP) para un agente individual.

    Retorna:
        dict: {"success": bool, "result": dict|None, "error": str|None}
              - result contiene el JSON parseado de la respuesta final.
    """
    try:
        client = OpenAI(
        ...
        for iteration in range(max_iterations):
            ...
            if not message.tool_calls:
                ...
                try:
                    ...
                    return {
                        "success": False,
                        "result": None,
                        "error": f"JSON inválido: {str(parse_err)}. Respuesta cruda: {raw_content[:500]}",
                    }
            ...
            for tool_call in message.tool_calls:
                ...
                execution_log.append({
                    "step": f"{icon} [{agent_label}] Ejecutando: {func_name}",
        ...
        execution_log.append({
            "step": f"⚠️ {agent_label}: Límite de iteraciones alcanzado",
            "detail": "El agente no completó su tarea en el número máximo de iteraciones.",
        })
        return {
            "success": False,
            "result": None,
            "error": "El agente excedió el límite de iteraciones.",
        }
        ...
        return {
            "success": False,
            "result": None,
            "error": f"Error al conectar con el LLM: {str(e)}",
        }


def agent_analyze(ticket_data: dict, execution_log: list) -> dict:
    """
    Agente Analista — Paso 1 del flujo Multi-Agente.

    Usa el toolkit MCP - Analyze para resumir, categorizar y leer directrices.
    Retorna un dict con: resumen, categoria, prioridad, guidelines.
    """
    user_prompt = (
        ...
        f"documentación antes de categorizar.\n\n"
    ...
    outcome = _run_agent(
        ...
        tools=MCP_TOOLS_ANALYZE,
        tool_functions=TOOL_FUNCTIONS_ANALYZE,
    ...
    required_keys = {"resumen", "categoria", "prioridad"}
    if not required_keys.issubset(result.keys()):
        return {
            "success": False,
            "result": None,
            "error": f"Faltan keys en el análisis. Esperadas: {required_keys}",
        }

    # Asegurar que guidelines exista
...
def agent_action(
    ...
    """
    Agente Ejecutor — Paso 3 del flujo Multi-Agente.

    Recibe el JSON del Agente Analista como contexto de entrada directo.
    Usa el toolkit MCP - Action para proponer la solución y enviar notificación.
    Retorna un dict con: respuesta_propuesta, notify_result.
    """
    analysis_json = json.dumps(analysis, ensure_ascii=False, indent=2)

    user_prompt = (
        f"Recibes el siguiente análisis estructurado del Agente Analista. "
        f"Procede con la resolución y comunicación usando las herramientas MCP disponibles.\n\n"
        f"**Análisis del Agente Analista:**\n{analysis_json}\n\n"
        f"**Datos originales del ticket:**\n"
        f"- Título: {ticket_data.get('titulo', 'N/A')}\n"
        f"- Descripción: {ticket_data.get('descripcion', 'N/A')}\n"
    )
    ...
    outcome = _run_agent(
        ...
        user_prompt=user_prompt,
        tools=MCP_TOOLS_ACTION,
        tool_functions=TOOL_FUNCTIONS_ACTION,
        execution_log=execution_log,
    ...
    result = outcome["result"]
    if "respuesta_propuesta" not in result:
        return {
            "success": False,
            "result": None,
            "error": "Falta la key 'respuesta_propuesta' en la respuesta del ejecutor.",
        }
...
def generate_ticket_response(ticket_data: dict, execution_log: list = None) -> dict:
    """
    Orquesta el flujo Multi-Agente secuencial:

    1. Agent for Analyze → resume, categoriza y lee directrices.
    2. Comunicación → el JSON del Analista se pasa al Ejecutor.
    3. Agent for Action → propone solución y envía notificación por Gmail.

    Manejo de errores: si el Agente Analista falla, el Ejecutor NO se ejecuta.

    Retorna:
        dict: Análisis combinado con keys: resumen, categoria, prioridad,
              respuesta_propuesta, notify_result, agent_errors.
    """
    if execution_log is None:
        execution_log = []

    execution_log.append({
        "step": "🤖 Iniciando flujo Multi-Agente",
        "detail": "Paso 1: Agent for Analyze",
    })

    # --- Paso 1: Agent for Analyze ---
    analyze_outcome = agent_analyze(ticket_data, execution_log)

    if not analyze_outcome["success"]:
        execution_log.append({
            "step": "❌ Agent for Analyze falló",
            "detail": analyze_outcome["error"],
        })
        return {
            "resumen": "Error en el Agente Analista.",
            "categoria": ticket_data.get("categoria", "N/A"),
            "prioridad": ticket_data.get("prioridad", "N/A"),
            "respuesta_propuesta": (
                f"⚠️ El Agente Analista no pudo completar el análisis.\n\n"
                f"**Detalle del error:** `{analyze_outcome['error']}`\n\n"
                f"El Agente Ejecutor no se ejecutó para evitar operar con datos vacíos."
            ),
            "notify_result": None,
            "agent_errors": {"analyze": analyze_outcome["error"]},
        }

    analysis = analyze_outcome["result"]
    execution_log.append({
        "step": "✅ Agent for Analyze completado — pasando contexto al Ejecutor",
        "detail": f"Resumen: {analysis.get('resumen', '')[:150]}",
    })

    # --- Paso 2: Comunicación entre Agentes (contexto directo) ---
    execution_log.append({
        "step": "🔄 Comunicando análisis al Agent for Action",
        "detail": "Enviando JSON estructurado como contexto de entrada.",
    })

    # --- Paso 3: Agent for Action ---
    action_outcome = agent_action(ticket_data, analysis, execution_log)

    if not action_outcome["success"]:
        execution_log.append({
            "step": "❌ Agent for Action falló",
            "detail": action_outcome["error"],
        })
        return {
            ...
            "respuesta_propuesta": (
                f"⚠️ El Agente Ejecutor no pudo completar la resolución.\n\n"
                f"**Análisis previo (Agente Analista):**\n"
                f"- Resumen: {analysis.get('resumen', 'N/A')}\n"
                f"- Categoría: {analysis.get('categoria', 'N/A')}\n"
                f"- Prioridad: {analysis.get('prioridad', 'N/A')}\n\n"
                f"**Detalle del error:** `{action_outcome['error']}`"
            ),
            "notify_result": None,
            "agent_errors": {"action": action_outcome["error"]},
        }

    action_result = action_outcome["result"]
    execution_log.append({
        "step": "✅ Flujo Multi-Agente completado",
        "detail": "Ambos agentes finalizaron correctamente.",
    })
    ...
    return {
        ...
        "categoria": analysis.get("categoria", "N/A"),
        "prioridad": analysis.get("prioridad", "N/A"),
        "respuesta_propuesta": action_result.get("respuesta_propuesta", "N/A"),
        "notify_result": action_result.get("notify_result"),
        "agent_errors": None,
    }
...
def panel_administracion() -> None:
    ...
    for ticket in tickets:
        with st.container():
            ...
            with col_ia:
                # Contenedor de IA — Multi-Agent Analysis (Workshop 05)
                with st.expander("🤖 Multi-Agent Analysis", expanded=False):
                    ...
                    if ticket["respuesta_ia"]:
                        try:
                            ...
                            # Mostrar resultado organizado en pestañas Multi-Agente
                            ...
                            with tab2:
                                st.markdown("**Propuesta del Agente Ejecutor:**")
                    ...
                    if st.button(
                        "🤖 Analizar con Multi-Agent (MCP)",
                        key=f"ia_{ticket['id']}",
                        type="secondary",
                    ):
                        if is_agent_configured():
                            # Usar el sistema Multi-Agente con Tool Calling (MCP)
                            execution_log = []
                            analysis = None

                            # --- Paso 1: Agent for Analyze ---
                            with st.status("🤖 AI Agent for Analyze trabajando...", expanded=True) as status_analyze:
                                st.markdown("*Consultando guías, resumiendo y categorizando...*")
                                analysis = generate_ticket_response(
                                    ticket, execution_log=execution_log
                                )
                                status_analyze.update(
                                    label="✅ AI Agent for Analyze completado",
                                    state="complete",
                                )

                            # --- Paso 2: Agent for Action ---
                            has_error = analysis.get("agent_errors") is not None
                            with st.status("⚙️ AI Agent for Action ejecutando...", expanded=True) as status_action:
                                st.markdown("*Generando respuesta y enviando notificación...*")
                                if has_error:
                                    status_action.update(
                                        label="⚠️ AI Agent for Action omitido (error en Analista)",
                                        state="error",
                                    )
                                else:
                                    status_action.update(
                                        label="✅ AI Agent for Action completado",
                                        state="complete",
                                    )
                            ...
                            elif is_smtp_configured() and not notify_info and not has_error:
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        st.markdown(
            ...
            f"Flujo: Problema → Multi-Agent → Solución<br/>"
