...
ejecutar un flujo de decisión ramificado en 4 acciones clave:

1. **Summarize (Resumir):** Genera un resumen ejecutivo del problema en un \
solo párrafo corto.
2. **Categorize (Categorizar):** Determina la categoría del ticket \
(Técnico, Bug, Facturación, Solicitud de feature) y su nivel de prioridad \
(Alta, Media, Baja).
3. **Propose (Proponer):** Genera una propuesta de respuesta detallada, \
clara y empática para el cliente.
4. **Notify (Notificar):** El sistema enviará una notificación por correo \
electrónico automáticamente después de tu análisis.
...
**IMPORTANTE:** Debes devolver ÚNICAMENTE un JSON válido con la siguiente \
estructura, sin texto adicional antes o después:
...
def generate_ticket_response(ticket_data: dict) -> dict:
    """
    Genera un análisis estructurado (JSON) para un ticket usando un LLM
    compatible con la API de OpenAI.

    El agente ejecuta 4 acciones: Summarize, Categorize, Propose, Notify.
    ...
    Retorna:
        dict: Análisis estructurado con keys: resumen, categoria,
              prioridad, respuesta_propuesta. O un dict de error.
    """
    ...
    try:
        ...
        try:
            # Limpiar el contenido: remover markdown code fences si existen
            clean_content = raw_content.strip()
            if clean_content.startswith("```"):
                # Remover ```json ... ``` o ``` ... ```
                lines = clean_content.split("\n")
                # Quitar primera y última línea (los fences)
            ...
            if not required_keys.issubset(result.keys()):
                raise ValueError(f"Faltan keys en el JSON. Esperadas: {required_keys}")

            return result

        except (json.JSONDecodeError, ValueError) as parse_err:
            return {
                "resumen": "No se pudo parsear la respuesta del LLM.",
                ...
                "respuesta_propuesta": (
                    ...
                    f"**Respuesta cruda del modelo:**\n\n{raw_content}\n\n"
                    f"**Error de parseo:** `{str(parse_err)}`"
                ),
            }
        ...
        return {
            ...
            "respuesta_propuesta": (
                ...
                f"(AGENT_API_URL, AGENT_API_KEY, AGENT_MODEL_NAME)."
            ),
        }
