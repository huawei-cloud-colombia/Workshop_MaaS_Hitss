...
# ============================================================================
# 6. AI AGENT — LLM compatible con OpenAI para respuesta automatizada
# ============================================================================

# --- System Prompt robusto para el agente ---
SYSTEM_PROMPT = """Eres un Especialista de Soporte Técnico Senior con más de 10 años \
de experiencia. Tu objetivo es analizar un ticket de soporte y generar una respuesta \
profesional, empática y accionable para el usuario.

Directrices:
1. Analiza el tono del ticket (frustrado, neutral, urgente) y ajusta tu empatía.
2. Identifica el problema central y resúmelo en una sola línea.
3. Proporciona pasos concretos y accionables (numerados) para resolver o diagnosticar.
4. Si necesitas más información, indícalo claramente al final.
5. Usa un lenguaje claro, sin jerga innecesaria. Incluye comandos técnicos \
solo si son relevantes.
6. Estructura tu respuesta con: saludo empático, resumen del problema, \
pasos sugeridos y cierre.

Formato de salida:
- **Saludo:** Reconoce al usuario y su situación.
- **Resumen:** "He identificado que el problema principal es..."
- **Pasos sugeridos:** Lista numerada.
- **Cierre:** Próximos pasos o escalamiento si aplica.
"""
...
def generate_ticket_response(ticket_data: dict) -> str:
    """
    ------------------------------------------------------------------
    AI Agent: genera una respuesta automatizada para un ticket de
    soporte utilizando un LLM compatible con la API de OpenAI.
    ------------------------------------------------------------------
    Args:
    ...
    if not is_agent_configured():
        return (
            ...
            "- `AGENT_API_URL`\n"
            "- `AGENT_API_KEY`\n"
            "- `AGENT_MODEL_NAME` (opcional)"
        )

    # --- Construir el mensaje del usuario con los datos del ticket ---
    user_message = (
        ...
        f"Genera una respuesta profesional para el usuario."
    )
    ...
    try:
        ...
        return (
            f"❌ **Error al contactar el AI Agent.**\n\n"
            f"No se pudo generar la respuesta. Detalle del error:\n"
            f"```\n{str(e)}\n```"
        )
...
def _render_ticket_card(ticket: dict) -> None:
    ...
    with st.container(border=True):
        ...
        with st.expander("🤖 AI Agent Analysis", expanded=False):
            ...
            # --- Mostrar respuesta existente del agente ---
            if ticket.get("respuesta_agente"):
                with st.chat_message("assistant"):
                    st.markdown(ticket["respuesta_agente"])

            # --- Botón para generar / regenerar ---
            if st.button(
                ...
                if not is_agent_configured():
                    ...
                    with st.spinner("El agente está analizando el ticket..."):
                        ...
                        guardar_respuesta_agente(ticket["id"], respuesta)
