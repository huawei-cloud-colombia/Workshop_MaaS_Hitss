...
Evolución: Integración de un AI Agent que analiza tickets y genera
respuestas automatizadas mediante un LLM compatible con OpenAI.

Tecnologías: Python + Streamlit + SQLite + OpenAI API
...
# ---------------------------------------------------------------------------
SYSTEM_PROMPT = """Eres un Especialista de Soporte Técnico Senior con años \
de experiencia resolviendo incidencias en entornos de desarrollo, cloud e \
infraestructura. Tu objetivo es analizar el ticket de soporte que recibes y \
generar una propuesta de respuesta automatizada para el usuario.

Directrices para tu respuesta:
1. Analiza el tono del ticket (urgente, frustrado, informativo) y responde con \
empatía y profesionalismo.
2. Identifica el problema central basándote en el título, categoría, prioridad y \
descripción.
3. Estructura una respuesta clara y accionable con pasos concretos.
4. Si el problema requiere escalamiento, indícalo explícitamente.
5. Mantén un lenguaje técnico preciso pero accesible.
6. Limita tu respuesta a un máximo de 300 palabras.

Formato de salida:
- **Análisis:** Breve identificación del problema.
- **Causa probable:** Hipótesis de la raíz del problema.
- **Pasos sugeridos:** Lista numerada de acciones concretas.
- **Escalamiento:** Indicar si es necesario o no, con justificación.
"""
...
def generate_ticket_response(ticket_data: dict) -> str:
    ...
    Parámetros:
        ticket_data (dict): Diccionario con los datos del ticket
            (titulo, categoria, prioridad, descripcion, estado).

    Retorna:
        str: La respuesta generada por el agente, o un mensaje de error.
    """
    # Construir el prompt del usuario con los datos del ticket
    user_prompt = (
        f"Analiza el siguiente ticket de soporte técnico y genera una respuesta:\n\n"
    ...
    try:
        ...
        return (
            ...
            f"No se pudo generar la respuesta automatizada. "
            f"Detalle del error: `{str(e)}`\n\n"
            f"Por favor, verifica la configuración del archivo `.env` "
            f"(AGENT_API_URL, AGENT_API_KEY, AGENT_MODEL_NAME) e inténtalo nuevamente."
        )
        ...
        with st.container():
            ...
            with col_ia:
                ...
                with st.expander("🤖 AI Agent Analysis", expanded=False):
                    ...
                    # Botón para generar respuesta con IA
                    if st.button(
                        ...
                        if is_agent_configured():
                            # Usar el AI Agent con LLM
                            with st.spinner("El agente está analizando el ticket..."):
                                respuesta = generate_ticket_response(ticket)
                        else:
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        st.markdown(
            ...
            "Flujo: Problema → Aplicación con IA → Solución Parcial<br/>"
